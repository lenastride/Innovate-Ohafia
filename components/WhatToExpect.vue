...
<template>
  <main class="flex flex-col justif-center items-center">
    <section class="w-full max-w-[70em]">
      <h1 class="font-bold text-2xl md:text-3xl text-[#D90000] ml-[6.3%] text-left">What to expect</h1>
    </section>
    
    <section class="flex flex-col w-full pt-10 md:flex-row justify-center items-center">
      <!-- Image Section - Slides through different images -->
      <div class="w-full overflow-hidden max-h-[34em] relative">
      ...
      <div class="w-full md:p-[5em] md:pl-[7em] bg-cover md:bg-[url(/who_to_expect_bg.png)] g-[url(/innovate_patter.png)] h-[34em] overflow-y-auto custom-scrollbar">
        <div class="">
          <div 
            v-for="(activity, index) in obj.activities" 
            :key="index"
            class="flex items-center cursor-pointer group relative pl-10"
            @click="handleUserInteraction(index)"
          ...
          <div 
              ...
              :class="currentSlide === index ? 'bg-[#D90000]' : 'bg-gray-300 group-hover:bg-gray-400'"
            ></div>
            <span class="block w-full text-2xl font-bold transition-colors duration-300   border-red-700"
                  :class="currentSlide === index ? 'text-[#D90000]' : 'text-gray-800'">
              {{ activity.name }}
            </span>
            <span class="block w-full font-bold text-[5rem] transition-colors duration-300"
                  :class="currentSlide === index ? 'text-[#D90000] font-extrabold' : 'font-bold text-outline-2'">
